...
print(Fore.GREEN+"For a step-by-step video guide, watch (coming soon).")

# input (for the gen file)
g_mode = input(Fore.WHITE+'Enter the script mode (exe or batch): ')
...
try: 
    o_filehost = input(Fore.WHITE+"Enter the source zip file host URL (you can use github): ")
    o_pack_count = input(Fore.WHITE+"Enter how many dependencies you want to install (0 for none): ")
    o_packs = []
    for i in range(int(o_pack_count)):
        o_packs.append(input(Fore.WHITE+f"[{i}] Enter the dependencie url: "))
    o_instal_dir = input(Fore.WHITE+r"Enter the directory you want your app to be installed in (things like %appdata% can be used): ")
except Exception as err:
    print(f'Exception while parsing input: \n'+str(err))
# magic
...
if g_mode=="batch":
    ...
    print("Error: invalid mode detected. Exitting...")